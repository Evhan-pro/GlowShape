...
export default function Contact() {
  ...
  const adresse = settings?.adresse || 'Espace Anjou';
  const ville = settings?.ville || '49000 Angers';
  const mapQuery = encodeURIComponent(`${adresse}, ${ville}`);
  ...
  return (
    <div data-testid="contact-page" className="min-h-screen pt-20 sm:pt-24 pb-12 sm:pb-16">
      <div className="container-custom px-4">
        ...
        >
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-serif mb-4">Contactez-nous</h1>
          <p className="text-sm sm:text-base text-muted-foreground max-w-xl mx-auto">
            N'hesitez pas a nous contacter pour toute question ou pour prendre rendez-vous.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 mb-8 sm:mb-12">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
            {success ? (
              <div className="bg-green-50 border border-green-200 rounded-sm p-6 sm:p-8 text-center">
                ...
                <h3 className="text-lg sm:text-xl font-serif mb-2">Message envoye !</h3>
                <p className="text-sm text-muted-foreground mb-4">Nous vous repondrons dans les plus brefs delais.</p>
              ...
              <form onSubmit={handleSubmit} data-testid="contact-form" className="bg-white border border-border rounded-sm p-5 sm:p-8 space-y-4 sm:space-y-5">
                <h2 className="text-xl sm:text-2xl font-serif mb-2">Ecrivez-nous</h2>
                <div>
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-medium mb-1.5">Telephone</label>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="space-y-6">
            <div className="bg-white border border-border rounded-sm p-5 sm:p-8 space-y-4 sm:space-y-5">
              <h2 className="text-xl sm:text-2xl font-serif mb-2">Nos coordonnees</h2>
              <div className="flex items-start space-x-3">
                <MapPin size={20} className="text-accent flex-shrink-0 mt-0.5" />
                <div>
                  <p className="font-medium text-sm">{adresse}</p>
                  <p className="text-muted-foreground text-xs sm:text-sm">{ville}</p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                <Phone size={20} className="text-accent flex-shrink-0 mt-0.5" />
                <p className="text-sm">{settings?.telephone || '01 23 45 67 89'}</p>
              </div>
              <div className="flex items-start space-x-3">
                <Mail size={20} className="text-accent flex-shrink-0 mt-0.5" />
                <p className="text-sm break-all">{settings?.email || 'contact@institut.fr'}</p>
              </div>
              <div className="flex items-start space-x-3">
                <Clock size={20} className="text-accent flex-shrink-0 mt-0.5" />
                <div className="text-sm">
                  <p className="font-medium">Horaires d'ouverture</p>
                  <p className="text-muted-foreground text-xs sm:text-sm">Lundi - Samedi : 9h00 - 19h00</p>
                </div>
              </div>
            </div>
